module.exports = function(roomModel, socket){
  ...
  socket.on('updateUsers', function(){
    setTimeout(function(){
      socket.in(joinedRoom.id).emit('userList', joinedRoom.users);
    }, 40);
  });
  ...
  socket.on('enemyAnswer', function(data){
    socket.broadcast.to(joinedRoom.id).emit('loseMessage');
  })
